...
public class PostService {
    ...
    public PageResponse<PostResponse> getMyPost(int page, int size){
        ...
        try {
            ...
            log.error("Error whie getting user profile", e);
        }
...
//                .map(postMapper::toPostResponse).toList();
        String userName = userProfile != null ? userProfile.getLastName() : null;
        var postList  =  pageData.getContent().stream().map(post -> {
            var postResponse = postMapper.toPostResponse((post));
            postResponse.setCreated(timeFormatter.format(post.getCreatedDate()));
